export class MaranhaoService {
  ...
  validar(inscricaoEstadual: string) {
    ...
    if (!this.isPrimeirosDigitosUmDois(ieSemMascara)
  ...
  private isPrimeirosDigitosUmDois(inscricaoEstadual: string) {
    // https://jex.im/regulex/#!embed=false&flags=&re=%5E24
    return /^12/.test(inscricaoEstadual);
  }
