export class MatoGrossoSulService {
  ...
  validar(inscricaoEstadual: string) {
    ...
    if (!this.isPrimeirosDigitosDoisOito(ieSemMascara)
  ...
  private isPrimeirosDigitosDoisOito(inscricaoEstadual: string) {
    // https://jex.im/regulex/#!embed=false&flags=&re=%5E28
    return /^28/.test(inscricaoEstadual);
  }
